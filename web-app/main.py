...
@app.get("/api/export/lit-review")
async def export_lit_review(project: str):
    """Bundle all 12-point extractions in a project into a single
    literature-review markdown file. Returns the file as a download.
    Useful for offline writing in Word, Obsidian, or any markdown editor.
    """
    if not project or project.startswith("00-"):
        raise HTTPException(400, "Valid project name required")
    ext_dir = PROJECTS / project / "02-EXTRACTIONS"
    if not ext_dir.exists():
        raise HTTPException(404, f"No extractions folder for {project}")
    files = sorted(ext_dir.glob("*.md"))
    if not files:
        raise HTTPException(404, f"No extractions found in {project}")

    today = datetime.date.today().isoformat()
    parts = [
        f"# Literature Review — {project}",
        f"*Generated by ResearchPilot on {today} · {len(files)} paper(s)*",
        "",
        "---",
        "",
    ]
    ...
    bundle = "\n".join(parts)
    audit("lit_review.export", project=project, papers=len(files), size=len(bundle))
    return HTMLResponse(
        content=bundle,
        media_type="text/markdown",
        headers={"Content-Disposition": f'attachment; filename="lit-review-{project}-{today}.md"'},
    )
